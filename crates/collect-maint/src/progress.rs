...
pub fn report(stage: &str, message: impl Display) {
    ...
    match status::mode() {
        status::StatusMode::Plain => eprintln!("[{}] {}", stage, message),
        status::StatusMode::Tui => status::update_message(stage, message),
    }
}
    ...
    match status::mode() {
        status::StatusMode::Plain => {
            if should_report(current, total, SCAN_REPORT_INTERVAL) {
                eprintln!("[{}] {}/{} {}", stage, count(current), count(total), detail);
            }
        }
        status::StatusMode::Tui => status::update_step(stage, current, total, detail),
    }
}

pub fn should_report(current: usize, total: usize, interval: usize) -> bool {
    current == total || current % interval == 0
}
